...
public function edit_Event(Request $request, $id)
{
    ...
    // Check and handle the image_one file upload
    if (isset($request->image_one)) {
        $fileSize = $request->file('image_one')->getSize();
        if ($fileSize >= 1800000) { // Size check
            Session::flash('message', "File Exceeded the maximum allowed Size");
            Session::flash('messageError', "An error occurred, You may have exceeded the maximum size for an image you uploaded");
            return Redirect::back();
        } else {
            $file = $request->file('image_one');
            $filename = str_replace(' ', '', $file->getClientOriginalName());
    ...
    // Update the event in the database using the DB facade
    DB::table('events')->where('id', $id)->update($updateDetails);
...
public function add_Experience(Request $request)
{
    ...
    function handleFileUpload($file, $path, $prefix) {
        $filename = str_replace(' ', '', $file->getClientOriginalName());
        $timestamp = new DateTime();
        $new_timestamp = $timestamp->format('Y-m-d H:i:s');
        $image_temp = $new_timestamp . $prefix . $filename;
        $image = str_replace(' ', '', $image_temp);
        $file->move($path, $image);
        return $image;
    }
    ...
    for ($i = 1; $i <= 6; $i++) {
        ...
        if (isset($request->$imageField)) {
            ...
            if ($file->getSize() >= $maxFileSize) {
                ...
                $images[$imageField] = handleFileUpload($file, $path, 'image');
            }
        } else {
            $images[$imageField] = $request->$imageCheat;
        }
    }

    // Ensure locations and durations exist
    $locationExists = DB::table('locations')->where('name', $request->location)->exists();
    if (!$locationExists) {
    ...
    $durationExists = DB::table('durations')->where('name', $request->duration)->exists();
    if (!$durationExists) {
        $duration = new Duration;
        $duration->name = $request->duration;
        $duration->save();
    }
...
public function add_Sample(Request $request)
{
    ...
    // Create a new Sample record
     ...
     DB::table('samples')->insert([
        ...
        'image_three' => $image_three,
        'image_four' => $image_four,
        'thumbnail' => $request->thumbnail, // Ensure you have this in your request or set a default value
        'created_at' => now(),
        'updated_at' => now(),
    ]);
